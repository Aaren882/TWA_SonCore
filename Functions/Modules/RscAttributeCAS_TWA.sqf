...
// Onload:
missionnamespace setVariable ["Display_TWAF_CAS_UI",[_params,_unit]];
...
if (_weaponTypesID < 2) then {
	{
		...
		_weapons_info = _x # 1;
		{
			...
			if (("autoLo" in _weapon_mode) or ("A10tastic_GAU8" in _weapon_mode)) then {
				call _call_Display;
			};
		...
		if (_weaponTypesID == 2) then {
			_weapons_Num = {
				...
				!(
					...
					((_x # 0) isKindOf ["weapon_LGBLauncherBase",_cfgweapon_path]) or
					((_x # 0) isKindOf ["missiles_SCALPEL",_cfgweapon_path])
				)
			} count _weapons_info;
		};
...
_fnc_onUnload = {
	...
	if ((_unit getvariable ["vehicle",""]) isEqualTo "") then
	{
		deleteVehicle _unit;
	};
};
